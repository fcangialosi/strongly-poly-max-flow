...
namely~\cite{Orlin1988}.}
	...
	Finally, in Section~\ref{sec:discussion}, we conclude with a brief summary of techniques utilized by both strongly polynomial algorithms and how the two algorithms differ.
...
%    Given optimal dual labels $\mu$, consider the primal problem relabeled by $\mu$.
  ...
  The optimal collection of tight edges encodes all the information necessary
    ...
    the algorithm spreads excesses around to keep them small.
		\question{This is talking only about the 2013 one, not about both, and says
		"the"}
    ...
    to use a node-wise certificate of contractibility than the edge-wise one described
		above.\question{Centralize definition of plentiful node here?}
...
and maintaining it throughout the algorithm.
...
find an initial feasible solution for the uncapacitated version by
following the transformation in \cite{article} (this just requires
adding a few additional edges and nodes).
Next, we need a feasible $\mu$: starting with 
$\mu_t \coloneqq 1$, we can use the CS property $\gij\mu_i = \mu_j$ to work
backwards computing the remaining $\mu$.
Finally, we can enforce that all edges with positive flow are tight by running 
the \textsc{Tight-Flow} subroutine, which is essentially a standard max flow operation on a modified graph (with lower/upper bounds and an added source). The new flow found in \textsc{Tight-Flow} is used to replace the flow values on each tight edge, while the flow on non-tight edges is set to zero. 
...
Once we have an initial conservative pair, we set the starting value of $\Delta$ equal to
...
This is where we note the importance of the $\sfrac{1}{n}$ threshold and the $\filtration$
subroutine. For this analysis, we will define D as the set of nodes with $\sfrac{\abs{b_i^\mu}}{\Delta} \geq \sfrac{1}{n}$ On each iteration, if there is not an augmentation (shrinking), then
...
The analysis above gives us a strongly polynomial bound on the total number of iterations. It takes at most $O(mnlogn)$ iterations to find a contractible edge and, since the stopping condition is contracting the graph down to one node, this happens O(n) times. The running time per iteration is dominated by $\filtration$, which takes O(nm) time. Therefore, this gives an upper bound of $O(n^3m^2\log n)$ for the total runtime (all other operations including initialization
