...
\section{Introduction}\label{sec:intro}
	...
	\frank{Check if the A projects all have related work sections.}
	\frank{Add related work at the end if we have enough space.}
...
\section{Preliminaries}\label{sec:prelim}
	...
	\frank{Rewrite: These can make the problem unbounded / cause problems / leave unbounded
	solutions. Can always increase the flow
	without violating any of the constraints. For the remainder of this discussion.
	we'll assume they don't exist. It's easy to eliminate them blah blah.}
...
\vspace{0.5cm}
	...
	optimal labelings.\frank{Double check throughout paper to make sure word label
	is used consistently.}
	\frank{Resassign to david: Reduces to problem of finding tight tree and that's inherently more
	combinatorial.}
...
\label{sec:2013}
    ...
    \label{sec:2013-notation}
		\frank{Do we want to add the tables back in?}
...
(1) through (4) below. Section~\ref{sec:runtime2013} provides analysis on the strongly polynomial runtime of this algorithm.
...
Following from primal-dual slackness, the optimality conditions for the primal are (i) $\nfiu - \biu = 0 \quad \forall i \in V-t$ and (ii) $\giij = 1$ if $f_{ij} > 0 \quad \forall ij \in E$. The first condition is achieved by slowly reducing an upper bound on the excess at each node as the algorithm continues. The second condition is achieved by finding and maintaining conservative pairs (Definition~\ref{def:conservative}) throughout the algorithm.
...
that edge is very large compared to $\Delta$. In this algorithm, it can be shown by flow decomposition techniques that once an edge has $f_{ij}^\mu \geq 17m\Delta$, then that edge must be tight in every dual optimal solution. However, rather than keeping track
of this ratio, the algorithm maintains a non-decreasing ratio
$\frac{\abs{b_i^\mu}}{\Delta}$.
\begin{claim}
If a node $i$ has $\abs{b_i^\mu} \geq 20nm\Delta$, then it must
have a contractible edge adjacent to it.
\end{claim}
\begin{proof}
We use proof by contradiction to show this must be the case. Suppose none of the edges adjacent to node i are contractible; this means that each edge ij and ji must have $f_{ij}^\mu < 17m\Delta$. Consider the two cases below and note that $m \geq n \geq d_i + 1$.
\begin{enumerate}
\item If $b_i^\mu > 0$, then $\nfiu - \biu = \sum_{j:ji \in E} \giij f_{ij}^\mu - \sum_{j:ij \in E} f_{ij}^\mu - b_i^\mu < 17d_im\Delta - 20nm\Delta < 0$. This contradicts the requirement $\nfiu - \biu \geq 0$ (f is a feasible flow).
\item If $b_i^\mu < 0$, then $\nfiu - \biu = \sum_{j:ji \in E} \giij f_{ij}^\mu - \sum_{j:ij \in E} f_{ij}^\mu - b_i^\mu > -17d_im\Delta + 20nm\Delta \geq 3nm\Delta + 17m\Delta$. This value is greater than the required upper bound, $(4d_i + 8)\Delta$, for $\nfiu - \biu$ though, which is a contradiction. 
\end{enumerate}
\end{proof}
While finding and contracting edges, the algorithm maintains several sets as defined below:
	...
	\end{align*}
Set L is initialized to contain all nodes satisfying the condition. Sets H, R, and D are initially empty. Nodes must be equal to the upper excess bound, $\nfiu - \biu \leq 4(d_i + 2)\Delta$, to enter set H, but remain in the set until they fall below the $(d_i + 2)\Delta$ bound.
...
paths because these paths are the highest-gain paths since $\gamma_{ij}^\mu = 1$ (recall the dual constrains $\gamma_{ij}^\mu$ to be $\leq 1$). If the algorithm augments, set R is reset to set H and the next iteration begins.
...
decreasing $\Delta$ by a factor of $\alpha > 1$ (step 3b above). The solutions to the two augmentation barriers and an intuitive explanation of their $\alpha$ values are explained below. Each solution puts forth a candidate $\alpha$ value that optimizes its impact; the actual $\alpha$ is chosen to be the minimum of the candidate $\alpha$ values so that no feasibility conditions are broken.
\begin{enumerate}[(i),itemsep=0mm]
\item Increasing $\mu$ for the
reachable nodes (i.e. nodes in set R) causes the ratio
$\gamma_{ij}^\mu = \gamma_{ij} \frac{\mu_i}{\mu_j} < 1$ 
to increase to $\gamma_{ij} \frac{\alpha\mu_i}{\mu_j}$  for edges from nodes in $R$
to nodes in $V \setminus R$. This can cause an edge between a reachable and a
non-reachable node to become tight (i.e. $\gamma_{ij} \frac{\mu_i}{\mu_j} = 1$), which may add a low excess node to the set of reachable nodes. $\alpha_1$ is chosen to be $min \{\frac{1}{\giij}: ij \in E[R, V \setminus R]\}$. Intuitively, we want to choose an $\alpha$ that will bring $\gamma_{ij} \frac{\mu_i}{\mu_j}$ as close to 1 as possible without going over 1.
\item Decreasing $\Delta$ decreases the lower threshold for $H$, which might create more high excess nodes. $\alpha_2$ is chosen to optimize a series of equations that aim to decrease $\Delta$ such that nodes will have excess equal (but not more than) the upper bound $4(d_i + 2)\Delta$.
\end{enumerate}
This $\es$ subroutine is the only part of the function that changes the $\sfrac{\abs{b_i^\mu}}{\Delta}$ ratio. 
\begin{claim}
...
to zero. This subroutine can speed up the finding the contractible arc process
...
$V \setminus R$ to $t$ continues until the graph is contracted into one node. The optimal solution, $\mu$, to the contracted instance of the dual can then be easily extended to the original instance. This is reverse function essentially undoes the contraction operations by assigning a $\mu'$ value to the non-representative node (i.e. if edge pq was contracted and p remained in the graph, assign $\mu'$ to q). The assigned $\mu'$ value is $\mu_p\gamma_{pq}$ if p=t or $\frac{\mu_q}{\gamma_{pq}}$ if p $\neq$ q. A final max flow is then run on the resulting optimal dual solution to get an optimal primal solution (Section~\ref{sec:motivate-dual}).
...
First note that since the stopping condition for the ``find contractible edge'' is reducing the graph to one node, there can only be n-1 contractions. Therefore the analysis aims to put a strongly polynomial bound on the iterations between contractions (i.e. prove a contractible edge can be found within a polynomial number of steps). %For analysis, we divide these iterations into the following categories:
...
The potential increases by $4(d_i + 2) - (d_i + 1) = 3d_i + 7$ when a node is added to H (\textit{expanding iteration}) and decreases by $(d_i + 2) - (d_i + 1) = 1$ when a node leaves H (\textit{shrinking iteration}). Once a node leaves H, $\Delta$ must double before that node can enter H again; this allows us to bound the total number of times a node can enter H. Since H is initially empty ($\Psi = 0$), we can use the bound to calculate the maximum possible increase in potential. Furthermore, since a node must have $\frac{e_i}{\Delta\mu} \geq d_i + 2$ to be in H, the potential must always be positive. This allows us to use the total increase in potential to bound the number of shrinking iterations because we can only decrease the potential by less than the amount we increase it. 

This gives us a strongly polynomial bound on the number of expanding and shrinking iterations; however, we must also prove that there are only a few \textit{neutral} iterations in between these expanding and shrinking iterations. This is where we note the importance of the set D and the $\filtration$ subroutine. On each iteration, if there is not an augmentation (shrinking), then either $\es$ or $\filtration$ and $\es$ will be called. If $\es$ is called, then this means set $V \setminus R$ has at least one node in D that will have its $\sfrac{\abs{b_i^\mu}}{\Delta}$ increased by $\alpha$. Since this node is in D, its $\sfrac{\abs{b_i^\mu}}{\Delta}$ value is at least $\frac{1}{n}$, which means it will become abundant in a strongly polynomial number of iterations. If $\filtration$ is called first, then the excess of nodes in set $V \setminus R$ will decrease enough such that $\es$ will cause one of these nodes to enter set D or these low excess nodes will enter set R, allowing an augmentation (shrinking iteration) on the next iteration. Note the first option can only happen n-1 times because once a node enters D, it cannot leave ($\sfrac{\abs{b_i^\mu}}{\Delta}$ is non-decreasing) unless it is contracted. 
...
%%%	If value of pseudoflow on edge is really high then optimal flow must have some flow on that edge becasue optimal flow has to be close enough to the pseudoflow
	...
	\david{Related back to 2013 algorithm: say something about how cancelling excess instead of spreading it is
	more efficient}
	...
	\david{
		Add intuition:
		Only flow from tight edges.
		If nothing flowing in, can't satisfy any demand. If there's positive demand
		it has to satisfy it because it's feasible.
	}
	\david{
		Add intuition:
		Whenever there's demand that is satisfied there's a path to satisfying it. 
		If there's no path to sending any flow in there then it's not going to need
		to send any. 
	}
...
%    \end{proof}
    ...
    \end{itemize}
		...
		\david{Add / rewrite intuition about djikstras}
...
%    that the algorithm can make progress in the second step. 
    ...
    \end{figure}
		\david{Please review this section for correctness. Feel free to change.}
		Intuitively, scaling $\alpha$ is
		akin to filling a basin with water. As illustrated in
		Figure~\ref{fig:alpha},	imagine a basin with various levels,
		where the height of the water represents $\alpha$, and the height of each
		level corresponds to the point at which a constraint in the above LP becomes
		tight. Scaling $\alpha$ past one of these points in a single iteration would break the constraint.
    ...
    an upper bound on the running time.
		...
		\frank{Make this more of a sketch than complete proof. Don't need to give
		all the cases.}
    \begin{lemma}
    ...
    \end{proof}
		\frank{Make this more of a sketch}
    \begin{lemma}
        \label{lem:scaling}
...
\iffalse
    ...
    \subsection{Scaling and Rounding} 
		\frank{Revisit this, hasn't been touched in a while}
