...
\vspace{0.5cm}
    ...
    to the original problem. \katie{which original problem?}
    \end{claim}
    \begin{claim}
...
\label{sec:motivate}
    ...
    Given optimal dual labels $\mu$, consider the equivalent relabeled primal problem.
    As $\giij = 1$ whenever $f_{ij} > 0$ (follows from (\ref{eqn.cs})), an optimal $f^{\mu}$ must satisfy
    \[ b^{\mu} \leq \nabla f^{\mu}
    = \sum_{j \in \din(i)} \giij \fu_{ij} - \sum_{j \in \dout(i)} \fu_{ji}
    = \sum_{j \in \din(i)} \fu_{ij} - \sum_{j \in \dout(i)} \fu_{ji}, \]
    Finding a maximum generalized flow thus reduces to solving an ordinary
    maximum flow problem on the tight graph.
    Therefore the dual solution reduces the primal problem
    to an ordinary maximum flow problem with demand constraints.
	...
	Combining the previous two sections, the heart (and running-time bottleneck)
	of both algorithms is a process
	of alternating between finding contractible edges and contracting them, in
	order to find a set of tight edges, which optimizes the dual. One of the main
	differences between the two algorithms is in 
	\textit{how} they find contractible edges, which
	we will describe in Sections~\ref{sec:2013} and~\ref{sec:2017}. This impacts
	their potential analysis and leads to the better bound in the 2017
	paper~\cite{Olver2017}.

	The alternating process repeats until either (1) the graph
	has just one node remaining\frank{what if it has non-zero demand?}, or (2) all remaining nodes have zero relabeled
	demand $\biu$\frank{intuitively why is this a stopping condition?}. 
	In either case, at the end we are left with a set of tight
	edges under $\mu$, $\tau(\mu)$, which have been contracted. Since the dual constraint $\gij\mu_i \le \mu_j$ must be tight\frank{why?}, knowing $\mu$ on a single node dictates what the value of $\mu$ must be on all other edges. Thus, if we know $u_t=1$\frank{why?} in the optimal solution, and we know the set of tight edges, then we can easily use these constraints and the $\gamma$s to compute the optimal dual solution $\mu^*$. 
    
	In the first case, when we are left with just a single node, one can intuitively think of the set of contracted edges as a spanning tree of the original graph, which uniquely defines $\mu$ at every node. 
    In the second case, when there is more than one node left, but all nodes have zero demand, one can intuitively think of the set of contracted edges as a spanning forest. Each node represents a connected component of the graph whose negative demands perfectly balance out the positive, and thus flow just within that component is able to completely satisfy the demand constraints. In this case, knowing these components ensures the constraints are all satisfied, but in order to calculate the \textit{value} of the maximum flow, one only needs to consider the component containing the sink $t$.
    
    In order to better understand this, consider the simple flow network in Figure~\ref{fig:ex-graph}. \ By inspection, we can see that the maximum flow $f^{*}$ should send 8 units of flow along the top edge and 8 units of flow along the bottom edge, resulting in 6 units at the sink. In Figure~\ref{fix:ex-spanning}, we show the three possible spanning trees for this graph. In (a), the dual is not feasible. (b) and (c) are both feasible, but the value produced by (c) is greater, and is thus optimal. 
    
    Now, given $\mu^*$, since all edges are tight, this is exactly a traditional maximum flow problem, and we can easily run a traditional strongly polynomial algorithm, such as Edmonds-Karp, in order to compute $f^{\mu^{*}}$. Finally, given $f^{\mu^{*}}$, we simply undo the labeling to compute the true flow value: $f^{*} = f^{\mu*}\cdot\mu^{*}$.
