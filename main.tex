...
A high-level overview of the steps for Végh's initial algorithm \cite{Vegh2013} are shown below. This algorithm is parameterized by a scaling factor $\Delta$, which controls the amount of flow that can be augmented and the threshold for determining which edges are contractible.

\begin{enumerate}[(i)]
\item Find an initial feasible flow $\bar{f}$
\item Run a cycle-canceling algorithm to get a conservative labeling, $\mu$ \todo{make sure this is defined in previous section and reference}
\item Find contractible edges and contract until stopping condition is met (\ref{sec:contract}) by either augmenting $\Delta$ units of flow, increasing (decreasing) $\mu$ ($\Delta$), or redistributing flow around isolated parts of the graph
...
Once a conservative pair (f, $\mu$) is found, the algorithm maintains it by requiring that excess of node i $\geq$ incoming flow on non-tight edges $\geq \Delta d_i$. This ensures that a feasible solution is always "reachable" because setting the flow on non-tight edges to zero still leaves non-zero excess on every node. 
