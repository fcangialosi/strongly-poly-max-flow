...
\end{tabular}
	...
	optimal labelings. 
	\david{We want to make sure it's clear why we're focusing on solving the dual instead of
		the primal. Mention (either here or in S3) that it reduces to problem of
		finding tight tree and that's inherently more combinatorial, or something
		along those lines...}
...
\label{sec:motivate}
    ...
    and it preserves the dual objective and constraints. An edge $e$ which is
    tight under any dual-optimal solution is accordingly called \textbf{contractible}.
...
%	preserves the objective and constraints.
	...
	An algorithm based on edge contractions identifies and contracts one contractible
	edge at a time. There can be at most $n - 1$ contractions
	before the graph is left with a single vertex. The set of edges identified
	over the course of the contractions is acyclic, and takes the form of
	either a spanning tree or a spanning forest depending on which of the following
	cases occurs. The set of tight edges can be used to reconstruct a dual solution.
	...
	solution on the contracted graph is optimal.
...
%    Given optimal dual labels $\mu$, consider the primal problem relabeled by $\mu$.
	...
	only the one with the highest gain is maintained.
	This is motivated by the primal problem: in
	...
	Végh \cite{Vegh2013}. Both algorithms use primal-dual pairs as certificates for
	contractibility.
...
\item Find contractible edges and contract until a stopping condition is met
	(\emph{cf.} Section \ref{sec:progress}(i)) in one of the following ways:
	\begin{enumerate}
...
To find an initial conservative pair $(f,\mu)$, a feasible flow $\bar{f}$ with
no flow-generating cycles must first be found. $\bar{f} \equiv 0$ is a trivial
feasible solution to the capacitated version of the primal; this can be used to
find an initial feasible solution for the uncapacitated representation by
following the transformation in \cite{article} (this just requires
adding a few additional edges and nodes). Flow-generating cycles are then
removed by the \textsc{maximum-mean-gain cycle-canceling} algorithm, introduced
by Goldberg et. al \cite{Goldberg:1991:CAG:105014.105022}. This is required
because there cannot be flow-generating cycles if there is a finite conservative
labeling $\mu$ as shown in Claim~\ref{claim:nocycles}. The initial conservative
labeling is then determined by defining $\mu_t \coloneqq 1$ and then defining
$\mu_i \coloneqq$ (max $\gamma$ along path from $i$ to $t$)$^{-1} \quad \forall i \in
V-t$. Finally, a max flow procedure is run to find a generalized flow $f$ with
respect to $\mu$, which gives us a conservative pair $(f,\mu)$. At the end of
this initialization phase, we also set the starting value of $\Delta$ equal to
...
Once a conservative pair $(f, \mu)$ is found, the algorithm maintains a relaxed
version of the pair called a $\Delta$-conservative pair (or equivalently a
$\Delta$-feasible pair) such that $\nfiu - \biu \leq 4(d_i + 2)\Delta \quad
\forall i \in V-t$. This key new idea allows a bounded amount of flow to pass
along non-tight edges, while maintaining a ``security reserve''. The security
reserve requires that the excess of a node $i$ must be $\geq$ the incoming flow on
non-tight edges; this keeps a conservative pair within reach because setting the
flow on all non-tight edges to zero still leaves non-zero excess on every node
and results in a feasible flow with no flow-generating cycles. Additionally,
$\Delta$-conservative pairs require that the flow on a non-tight edge $(i,j)$ must be
$\leq \Delta$. As $\Delta$ decreases, the amount of flow on these non-tight
edges decreases and the amount relabeled excess that is created when the
non-tight edges are reset to zero also decreases. This means the algorithm gets
...
\end{claim}
\begin{proof}
Suppose, for sake of contradiction, none of the edges adjacent to $i$ are
contractible; this means that each edge $(i,j)$ and $(j,i)$ must have $f_{ij}^\mu < 17m\Delta$.
Consider the two cases below and note that $m \geq n \geq d_i + 1$.\katie{why?}
\begin{enumerate}[itemsep=0mm]
\item 
		If $b_i^\mu > 0$, then $$\nfiu - \biu = \sum_{j:ji \in E} \giij f_{ij}^\mu -
		\sum_{j:ij \in E} f_{ij}^\mu - b_i^\mu < 17d_im\Delta - 20nm\Delta < 0.$$ This
		contradicts the requirement $\nfiu - \biu \geq 0$ (f is a feasible flow).
\item If $b_i^\mu < 0$, then $$\nfiu - \biu = \sum_{j:ji \in E} \giij f_{ij}^\mu - \sum_{j:ij \in E} f_{ij}^\mu - b_i^\mu > -17d_im\Delta + 20nm\Delta \geq 3nm\Delta + 17m\Delta.$$
This value is greater than the required upper bound, $(4d_i + 8)\Delta$, for $\nfiu - \biu$ though, which is a contradiction. 
\end{enumerate}
\end{proof}
...
fall below the $(d_i + 2)\Delta$ bound.

\frank{Re-organize to match up with previous section without repeating too much}
\textbf{\textsc{Augmentation}} As shown in (3a) above, each iteration of the main while loop (the ``finding and contracting edge''
...
the goal is to push flow directly to the sink from unreachable
\katie{What is the definition of unreachable? If it's given somewhere already
	and I missed it, maybe just point back to it. Also, if it's "unreachable" how
can we push flow from it? Seems counter-intuitive..}
high excess
...
$p=t$ or $\frac{\mu_q}{\gamma_{pq}}$ if $p \neq q$. A final max flow is then run
...
to be in $H$, the potential must always be positive. This allows us to use the
