...
\label{sec:motivate}
    ...
    constraint for the relabeled optimum flow $f^\mu$ takes the form\frank{@david: can you double check the following equation? I think a few of the ij were flipped so I tried to fix it}
    \[ b^{\mu} \leq \nabla f^{\mu}
    = \sum_{(i,j) \in \din(i)} \gamma_{ji}^{\mu} \fu_{ji} - \sum_{(i,j) \in \dout(i)} \fu_{ij}
    = \sum_{(i,j) \in \din(i)} \fu_{ji} - \sum_{(i,j) \in \dout(i)} \fu_{ij}. \]
