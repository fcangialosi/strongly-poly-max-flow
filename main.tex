...
\label{sec:motivate}
	...
	\subsection{Contraction Certificates}
    ...
    \begin{claim} $h$ is acyclic. \end{claim}
    ...
    Now $h$ may be decomposed into a set of paths. Because $\gamma^\mu \leq 1$ on
    $E_g$, flow is only lost along these paths. As such, the total flow deposited at
    the ends of paths is at most the total flow leaving nodes. This is equal to
    \[ \sum_i \max\,\{0, -\nabla h_i^\mu\}
     = \sum_i \max\,\{0, \nabla g_i^\mu - \nabla (g^*)_i^\mu\}
     \leq \sum_i \max\,\{0, \nabla g_i^\mu - \biu \}
     = \Ex(g, \mu). \qedhere \]
...
\item Find an initial feasible flow $\bar{f}$ and set the initial $\Delta$ to
	  $\max_{i \in V\setminus T} \nfiu - \biu$\frank{why this value?}
\item Run a cycle-canceling algorithm to get a conservative labeling, $\mu$\frank{maybe a quick reminder about why we want a conservative labeling?} 
\item Find contractible edges and contract until a stopping condition is met
	(\emph{cf.} Section \ref{sec:progress}) \frank{what's the stopping condition? is it not specific
	to the 2013 paper?} in one of the following ways:\frank{how does it choose
...
The algorithm maintains conservative labelings throughout\frank{why is this important?}, which require
feasible solutions to (P) and (D) as well as the complementary-slackness
property $\giij=1$ whenever $f_{ij}>0 \quad \forall ij \in E$. An initial
feasible flow can be found by first setting $f$ to zero in the capacitated version
and then finding the equivalent $f$ in the uncapacitated representation.
\frank{why is this necessary? why can't we just set $f=0$ in the uncapcitated
version directly?}
A conservative labeling, $\mu$, for $f$ can then be found by running a
cycle-canceling algorithm.\frank{why does this make it conservative?}

Once a conservative pair $(f, \mu)$ is found, the algorithm maintains it by
requiring that excess of node $i \geq$ incoming flow on non-tight edges. This
ensures that a feasible solution without any flow-generating cycles is always
``reachable'' because setting the flow on non-tight edges to zero still leaves
non-zero excess on every node. \todo{explain better}
...
\frank{Not crucial, but consider giving these more intuitive variables. I think
the worst is $T_0$ and $T$, I think it's confusing that they use the same
letter.}
	\begin{align*}
...
On each iteration of the main while loop (the ``finding and contracting edge''
while loop), the algorithm first looks to augment $\Delta$ units of flow on
tight paths from high relabeled excess nodes in set $T_0$ to low relabeled
excess nodes (including the sink) in set N. The algorithm only augments on tight
paths because these paths are the highest-gain paths since $\gamma_{ij}^\mu = 1$.
\frank{We need to make sure we mention somewhere that we do X,Y,Z so that we can
assume networks are lossy}
...
The subroutine $\es$ resolves both potential problems by increasing $\mu$ and
decreasing $\Delta$ by a factor of $\alpha > 1$\frank{How is this value chosen?}. Increasing $\mu$ for the
non-reachable nodes (i.e. nodes in set V $\setminus$ T) causes the ratio
$\gamma_{ij}^\mu \frac{\mu_i}{\mu_j} < 1$ 
\frank{What is this ratio and why does it have to be strictly less than 1?}
to increase for edges from nodes in $T$
to nodes in $V \setminus T$.
\frank{It might be helpful to refer to this as edges crossing the V-T cut}
...
hand, decreasing $\Delta$ decreases the lower threshold for $T_0$, which might
...
\end{claim}
\begin{proof}
$\alpha > 1$ by construction. The ratio for nodes in set $T$ is scaled by $\frac{\alpha}{\alpha}=1$ since
both the $\mu$ and $\Delta$ values are scaled. The ratio for nodes in 
the set $V\setminus T$ is multiplied by a factor of $\alpha > 1$ because only 
...
if $D \cap V\setminus T \neq \varnothing$. Since $\es$ only increases
$\sfrac{\abs{b_i^\mu}}{\Delta}$ for nodes in $V\setminus T$, the subroutine will
...
$\filtration$ runs a modified max flow iteration on nodes in set $V \setminus T$;
the goal is to push flow directly to the sink from unreachable high excess
nodes. The flow values computed in the max flow replace the current flows on
edges contained in $V \setminus T$; all other edge flow values remain the same
except for flow on edges going from nodes in $V \setminus T$  to $T$ which are set
...
\begin{enumerate}[(i),itemsep=0mm]
\item Setting the flow to zero on edges from nodes in $V\setminus T$ to $T$
	reduces the excess of nodes in $T$, which could cause the nodes to become
	low-excess nodes that are reachable to augment to in future iterations
\item Reducing excess of nodes in $V \setminus T$ might cause some of these nodes
...
$V \setminus T$ to $t$ continues until the graph is contracted into one node. 

\subsection{Strongly Polynomial Bound}
\frank{I think we at least need to say something tying this whole section back to a strongly polynomial runtime. If on top of that we can also say something about how they got the runtime, even if it's not too detailed, I think that'd be better.. }
...
%%%	If value of pseudoflow on edge is really high then optimal flow must have some flow on that edge becasue optimal flow has to be close enough to the pseudoflow
    ...
    Lemma \ref{lem.bound-dist2} leads naturally to a concrete characterization of contractible edges:
