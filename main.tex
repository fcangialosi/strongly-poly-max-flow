...
%%%	If value of pseudoflow on edge is really high then optimal flow must have some flow on that edge becasue optimal flow has to be close enough to the pseudoflow
	...
	nodes (roughly an $O(n^3)$ operation), cancelling excess only requires identifying
	a corresponding deficit for each excess. This is an informal argument for why the
...
%    \end{proof}
    ...
    of the numbers used in the algorithm, but it is beyond the scope of this exposition.)
