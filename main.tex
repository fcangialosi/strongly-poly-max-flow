...
\end{claim}
\begin{proof}
We use to proof by contradiction to show this must be the case. Suppose none of the edges adjacent to node i are contractible; this means that each edge ij and ji must have $f_{ij}^\mu < 17m\Delta$. Consider the two cases below and note that $m \geq n \geq d_i + 1$.
\begin{enumerate}
\item If $b_i^\mu > 0$, then $e_i^\mu = \sum_{j:ji \in E} \giij f_{ij}^\mu - \sum_{j:ij \in E} f_{ij}^\mu - b_i^\mu < 17d_im\Delta - 20nm\Delta < 0$. This contradicts the requirement $e_i \geq 0$ (f is a feasible flow).
\item If $b_i^\mu < 0$, then $e_i^\mu = \sum_{j:ji \in E} \giij f_{ij}^\mu - \sum_{j:ij \in E} f_{ij}^\mu - b_i^\mu > -17d_im\Delta + 20nm\Delta \geq 3nm\Delta + 17m\Delta$. This value is greater than the required upper bound, $(4d_i + 8)\Delta$, for $e_i^\mu$ though, which is a contradiction. 
\end{enumerate}
\end{proof}
...
decreasing $\Delta$ by a factor of $\alpha > 1$ (step 3b above). Increasing $\mu$ for the
...
to nodes in $V \setminus R$. \katie{describe how alpha value is chosen}
This can cause an edge between a reachable and a
non-reachable node to become tight (i.e. $\gamma_{ij} \frac{\mu_i}{\mu_j} =
...
contractible edge; however, this progress might be very slow. \katie{Is there
...
$20mn$ threshold; these nodes are included in set D.\katie{Same thing as before:
why this value?}
...
\katie{finish section and possibly cut out some definitions of iterations if need to save space}In this section we analyze a few key ideas that lead to a strongly polynomial bounded runtime. The full (and lengthy) set of proofs for the runtime analysis can be found in the original paper \cite{article}.
