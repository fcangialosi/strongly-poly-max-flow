...
\label{sec:motivate}
    ...
    primal solution may send flow (the tight graph).
    Moreover, these edges have relabeled gains $\gamma^\mu_e = 1$. Given an optimal dual solution,
    finding a maximum generalized flow thus reduces to solving an ordinary
    maximum flow problem on the tight graph.
    Starting from dual labels $\mu$, $f^{\mu}$ must satisfy
