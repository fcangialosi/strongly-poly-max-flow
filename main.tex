...
\iffalse
	...
	$f$ integral, and $f$ has bounded excess, as required in Theorem \ref{thm:2017-contraction}.
	
	To do this, the algorithm starts by computing a conservative pair $(f, \mu)$, ensuring
	...
	Next, $\mu$ is scaled so that the excess and deficit are uniformly bounded:
	\[ \biu - 1 \leq \nfiu \leq \biu - 2. \]
	Then, the flow
	is ``rounded'' to be integral by computing an ordinary flow $f'^\mu$ on the tight edges
	defined by $\mu$ under the constraints
	\[ \lfloor \nfiu \rfloor \leq \nabla (f')_i^\mu \leq \lceil \nfiu \rceil. \]
	The first epoch begins with the fitting pair $(f', \mu)$.
