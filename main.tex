...
\section{Introduction}\label{sec:intro}
	...
	or physical dissipation. The generalized maximum
	...
	that is faster than the original algorithm by a factor of
...
\section{Preliminaries}\label{sec:prelim}
	...
	\frank{I want to come back to this and reread prior work because i'm not
		totally sure that the unbounded thing is true -- also we don't assume they
		do not exist because later we talk about cancelling them, need to set this
	up better.}
...
\label{sec:motivate}
	...
	interior point methods to the linear program (\ref{eqn:primal}).
	\frank{Let's add a citation here}
...
%    Given optimal dual labels $\mu$, consider the primal problem relabeled by $\mu$.
	...
	than (c). (Note that the sign is negative because of our change of variable
	in~\hyperref[eqn:dual]{D}). Solving a max flow on just the tight edges of (b)
  ...
  The optimal collection of tight edges encodes all the information necessary
    ...
    to use a node-wise certificate of contractibility than the edge-wise one described
		above.\frank{Seems out of place. Should be moved somewhere else}
...
\item Run a cycle-canceling algorithm to get a conservative labeling, $\mu$.
	\frank{Still need to talk about this somewhere}
\item Find contractible edges and contract until stopping condition is met
	(\emph{cf.} Section~\ref*{sec:motivate-dual} Case~\ref{case:contract-tree}).
	\david{Any idea why (ii) is not possible in 2013? If yes and it's short,
	please add, if not, just delete this and skip it.}
...
$\bar{f}$, \rewrite{eliminate any cycles}, calculate the initial labels using $\mu_t
...
\rewrite{The algorithm maintains} a relaxed \textbf{$\Delta$-conservative pair},
...
\frank{We really need to clarify these numbers}
...
seeks to spread excess from $H$ to $L$:

\begin{itemize}
\item \textbf{Augmentation:} Each iteration of step (3) first looks to augment $\Delta$ units of flow on
tight paths from $H$ to $R \cap L$. This redistributes excess
so as to balance it out. After augmentation,
$R$ is reset to $H$ and the next iteration begins. \katie{Why is $R$ reset to $H$? Couldn't
there still be nodes in $R$ which are neither in $H$ nor in $L$?}
...
\item \textbf{Scaling:} The scaling phase resolves both potential problems by increasing $\mu$ in $R$ and
decreasing $\Delta$ by a factor $\alpha > 1$ (step (3b) above). 
\begin{enumerate}[(i),itemsep=0mm]
...
\end{proof}
\item \textbf{Redistributing Excess:} If $V \setminus R$ does not contain any
nodes with large values of $\sfrac{\abs{b_i^\mu}}{\Delta}$,
then scaling will be ineffective in producing a plentiful node.
\frank{Plentiful not defined yet}
As such, if $\sfrac{\abs{b_i^\mu}}{\Delta} < \sfrac{1}{n}$ for every node
\frank{Need to be careful and consistent about writing style of "we" vs "the
	algorithm" throughout}
in $V \setminus R$, we delay scaling and focus on redistributing the excess in $V \setminus R$.
\frank{Confusing because we say that augmentation also redistributes excess}
We compute a modified max flow along tight edges in $V \setminus R$.
The goal is to push flow directly to the sink from ``isolated''
nodes (nodes that are not reachable along a tight path from $H$).
The computed max flow values replaces the current flow on
...
the analysis aims to show a contractible edge can be found within a strongly polynomial time.
We define the following potential function to bound the number of iterations of step (3) defined at the beginning of Section~\ref{sec:2013}.
...
\rewrite{This gives us strongly polynomial bounds on the number of expanding and
...
\katie{Could this be moved so that the importance of $D$ is clearer
when you define it? Or could it be cut down?}
