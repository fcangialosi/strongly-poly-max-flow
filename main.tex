...
%    Given optimal dual labels $\mu$, consider the primal problem relabeled by $\mu$.
  ...
  The optimal collection of tight edges encodes all the information necessary
    ...
    a more useful contractibility certificate by disentangling the two. Both algorithms
    detailed below depend on a node-wise contractibility certificate in which the
...
a standard max flow operation on a modified graph (with lower/upper bounds and an added source)
...
$\sfrac{\abs{b_i^\mu}}{\Delta}$ increased by $\alpha$. Since this node is in D, $\sfrac{\abs{b_i^\mu}}{\Delta} \geq \sfrac{1}{n}$, which means
...
%     definition serves in place of abundant edge, and it only depends on the dual variables.
    ...
    to the current primal-dual solution pair $(f, \mu)$ if its relabeled absolute
