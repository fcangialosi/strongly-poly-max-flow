...
\katie{I'm a little weary of just stating this bound without any clarification
or intuition. -frank}
...
reduces the excess of nodes in $R$, which might open them up to accpet augmentations
in future round.
\end{itemize}
...
added to $H$ (this is called an \textit{expanding iteration}) \katie{What delimits a single iteration?}
and decreases by $(d_i + 2) - (d_i + 1) = 1$
when a node leaves $H$ (a \textit{shrinking iteration}). Once a node leaves
$H$, $\Delta$ must double \katie{Isn't $\Delta$ monotonically decreasing?}
before that node can enter $H$ again; this allows us to
...
\katie{What is considered one iteration? Is this the number of augmentations like in the analysis
in section 5?}
