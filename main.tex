...
\item Find contractible edges and contract until a stopping condition is met
	(\emph{cf.} Section \ref{sec:motivate-dual}(i)). Contractible edges are found
...
achieved by finding and maintaining conservative pairs
(Definition~\ref{def:conservative}) throughout the algorithm.
...
%     idea from Section \ref{sec:2013-notation}. \katie{could reduce this intro section since info already contained elsewhere}
	...
	conservative pair from Section \ref{sec:cert-contract}, wherein $f$ is not required
    to be feasible:
	\begin{definition}
	...
	cut property. To simplify the exposition, we define 
	\begin{definition}
