...
\vspace{0.5cm}
	...
	$\mu$, $\{\geu = 1\ \forall\ e  \in E\}$, as $\tau(\mu)$.
...
\label{sec:motivate}
	...
	\todo{We have not described what fitting is}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			T^- &= \{i \in V \mid \nfiu < \biu \}.
		\end{align*}
		Consider the residual graph of $f$ in the \rewrite{tight graph}. If there were a path
    ...
    \end{lemma}
    \begin{proof}
			As $g$ fits $\mu$, $g$ is \rewrite{supported} on the tight edges $\tau(\mu)$. As
...
\label{sec:13}
	...
	(ii) $\mu_t = 1$; and (iii) $\mu_i > 0, e_i \geq R_i$ for all $i \in V \setminus t$ where is $e_i \coloneq \sum_{j:ji \in E}\gamma_{ji}f_{ji} -  $ is defined as the excess of node i for a flow vector $f$.
	\todo{What's e?}
	...
	\subsection{Algorithm Overview} \label{sec:13-overview}
	\begin{table}[H]
	\begin{center}
    ...
    in Végh's full version of the paper~\cite{article}.
    Section~\ref{sec:cs} explains
 	the overall continuous scaling concepts in more detail and Sections
    \ref{sec:tf}-\ref{sec:filtration} analyze the three main subroutines used in this algorithm.
	 
	\subsection{Continuous Scaling Details}\label{sec:cs}
    ...
    are given in \cite{article}). \tf\ is then run one last time to convert the
	...
	\item Edges entering $T$: f$_{ij}$ = 0
...
introduces several new conceptual ideas that combine the techniques and insight from Section~\ref{sec:motivate}.
...
\begin{tabular}{ | p{7cm} | p{7cm} | p{2cm} | }
    ...
    No guarantee of abundant edge appearing in strongly polynomial number of
		steps \cite{Radzik2004} &  Continuous scaling & \\ \hline
...
\end{table}
	...
	on the \rewrite{tight subgraph}. The algorithm can then avail itself of the previously developed
