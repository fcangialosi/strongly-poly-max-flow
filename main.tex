...
%    Given optimal dual labels $\mu$, consider the primal problem relabeled by $\mu$.
	...
	but (c) produces a greater dual objective value ($-6$)
...
%the \textsc{Tight-Flow} subroutine, which is essentially 
...
%. The new flow found in \textsc{Tight-Flow} is used to replace the flow values on each tight edge, while the flow on non-tight edges is set to zero. 
%which involves running a
%traditional max-flow on a slightly modified $G$ and setting the flow to zero for
%all non-tight edges.
%calculate the initial labels using $\mu_t
...
The analysis above gives us a strongly polynomial bound on the total number of
iterations. It takes at most $O(mn\log n)$ iterations to find a contractible
edge, and there are at most $O(n)$ iterations (due to the stopping condition). The running
time per iteration is dominated by $\filtration$, which takes $O(nm)$ time.
Therefore, this gives an upper bound of $O(n^3m^2\log n)$ for the total runtime
...
%     definition serves in place of abundant edge, and it only depends on the dual variables.
	...
	\begin{proof}
		...
		\[ -b_i^\mu - \Ex(g, \mu) > 3n(d_i + 1) - 2n > 3nd_i > 3n|\din(i)| \]
		So there is some edge $e \in \din(i)$ with
...
%    \end{proof}
    ...
    by augmenting as much flow as possible, one unit at a time, along tight paths from nodes with
		excess to nodes with deficit. (Augmenting along tight paths maintains a fitting pair.)
        Formally, let $T$ be the union of the sink $t$ and all nodes with 
		deficit ($\nabla f_i^{\mu} < b_i^{\mu}$) and let $S$ (``sources'') be the
		set of all nodes $i$ for which $T$ is reachable from $i$ along a tight path
		in the residual graph. Continue augmenting from $S$ to $T$
        as long as there is some $i \in S$ for which the following conditions hold:
		%Let $S$ (``sources'') be
    ...
    phase must be cancelled by augmentation. In order to make this possible,
		only nodes in the ``reachable'' set $S$ are scaled. 
...
%    that the algorithm can make progress in the second step.
	...
	causes $i$ to be added to $S$, as $T$ is now reachable along a tight
	...
	node or
	one that was there previously) either becomes plentiful or is scaled to the point
    where its relabeled excess is 1, for example at label (d) in the figure.
	In the latter case, augmentation can make progress, so the next
	round of augmentation begins.
...
%	constraint becomes tight.
    ...
    In practice, $\alpha$ and $S$ can be updated via a modified Dijkstra's algorithm as follows.
    ...
    \end{proof}
    Because $f^\mu$ is always integral and excess is always immediately augmented away,
    the excess at every node stays bounded by $2$.
%    \begin{lemma}
%        \label{lem:scaling}
...
%    \end{proof}
	...
	Only the scaling rounds modify $\mu$, so it suffices to show that one scaling step,
    ...
    Suppose $b^\mu(X \cap S) > 0$. Let $E[S]$ denote the set of edges with both endpoints in $S$.
    Observe that $\din(X \cap S) \subseteq (\din(X) \cap E[S]) \cup \din(S)$.
    ...
    In summary, we have shown that augmentation and scaling maintain a fitting pair $(f, \mu)$ with
    $\mu$ safe and $\Ex(f, \mu)$ bounded, while increasing $|\biu|$. This will eventually produce a plentiful node.
...
\iffalse
    ...
    described in Section \ref{sec:sub-ppn},
    ...
    \Phi(f, \mu) &= \Psi(\mu) + \sum_{i \in \vsrc} \nfiu = \sum_{i \in \vsrc} \nfiu - \biu
    \end{align*}
    ...
    may end in $\vsrc$. All augmentations start in $\vsrc$, so
    all other augmentations decrease $\Phi$. So after $r$ augmentations,
...
Both algorithms take a combinatorial approach by computing a set of tight edges consistent
