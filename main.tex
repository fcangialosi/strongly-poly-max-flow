...
decreasing $\Delta$ by a factor of $\alpha > 1$. Increasing $\mu$ for the
...
\end{claim}
\begin{proof}
The ratio for nodes in set $T$ is scaled by $\frac{\alpha}{\alpha}=1$ since
both the $\mu$ and $\Delta$ values are scaled. The ratio for nodes in 
the set $V\setminus T$ is increased by a factor of $\alpha$ because only 
$\Delta$ is scaled. 
\end{proof}
